...
(function ($,window,document) {
	...
	$.extend($.Taiji,{
		...
		prototype : {
			...
			validationTip :function(ejson,parent){
				...
				$.each(ejsonContent,
						function(i, n) {
							...
							if(eles.length>1){
								$("div[data-for='"+i+"']",parent).tjPopover(n);
							}else {
								$("[name='"+i+"']",parent).tjPopover(n);
							}
					});
			},
			/******************************************************************
			 ...
			 * 封装了获取url、confirm,获取参数、加载中、校验、警告框、错误处理，成功框交由调用者弹出。
			 * 
	...
	$.Taiji.extendMethod({
		...
		eventHandler:{
			...
			_handleModal:function(element,size){
				...
				$.Taiji.showLoading($.Taiji.Messages.MSG_OPERATE);	
				var $this=this;
				$(element).showModal({"size":size,complete:function(){
						$this.isRunning = false;
						$.Taiji.hideLoading();
					}
				});
...
(function($){
	$.fn.extend({
		...
		tjPopover:function(content, options) {
			// 如果元素不可见，不处理
			...
			control.popover({ 
					...
					.popover("show")
					.one("click.taiji focus.taiji",function(){
						$(this).popover("hide");
						var control=$(this);
						var errorEle=control.parent();
